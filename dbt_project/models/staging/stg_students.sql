-- =============================================================================
-- Staging: stg_students
-- Purpose : Cast, rename, and lightly clean the raw Bronze students table.
-- =============================================================================

WITH source AS (
    SELECT * FROM {{ source('bronze', 'estudiantes_inscritos_2022') }}
),
...
renamed AS (
    SELECT
        ...
        END                                                        AS semester,
        CASE 
            WHEN CAST("inscritos" AS TEXT) ~ '^[0-9]+$' THEN CAST("inscritos" AS INTEGER)
            ELSE NULL 
    ...
    -- Filter using text cast for the regex check
    WHERE CAST("código_de_la_institución" AS TEXT) ~ '^[0-9]+$--'
      AND CAST("código_snies_del_programa" AS TEXT) ~ '^[0-9]+$'
)
